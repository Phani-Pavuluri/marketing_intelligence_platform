...
def test_starter_answers_are_distinct_substantive_and_safe() -> None:
    ...
    assert "historical channel measurement" in rendered[0]
    assert "channel spend" in rendered[1]
    assert "does not design GeoX tests" in rendered[2]
    assert "does not run live simulation or optimization" in rendered[3]
...
def test_preselection_answer_blocks_dataset_specific_claims() -> None:
    ...
    assert "Select the SaaS growth-planning example" in answer.next_action
