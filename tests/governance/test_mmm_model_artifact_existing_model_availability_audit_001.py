"""Governance checks for MMM model artifact and existing model availability audit."""
...
_AUDIT = Path("docs/audits/MIP_MMM_MODEL_ARTIFACT_AND_EXISTING_MODEL_AVAILABILITY_AUDIT_001.md")
_SUMMARY = Path(
...
_REQUIRED_EVIDENCE_PATHS = (
    ...
    "mip.evidence.registry",
    "mip.evaluation.gates",
    "mip.contracts.workflow_readiness",
    "mip.adapters.mmm",
)

_TRUE_FLAGS = ("audit_completed",)

_FALSE_FLAGS = (
    "mmm_model_artifact_contract_exists",
    "model_registry_exists",
    "existing_model_selection_supported",
    "existing_model_use_eligibility_supported",
    "requires_new_model_run_decision_supported",
)
...
def test_audit_states_coverage_level() -> None:
    content = _AUDIT.read_text(encoding="utf-8")
    assert "MISSING_NEEDS_NEW_EXISTING_MODEL_AVAILABILITY_GATE" in content
    assert "does **not**" in content or "does not" in content


def test_audit_states_recommended_next_artifact() -> None:
    summary = json.loads(_SUMMARY.read_text(encoding="utf-8"))
    assert summary["recommended_next_artifact"] == "MIP_MMM_EXISTING_MODEL_AVAILABILITY_GATE_001"
    content = _AUDIT.read_text(encoding="utf-8")
    assert "MIP_MMM_EXISTING_MODEL_AVAILABILITY_GATE_001" in content
