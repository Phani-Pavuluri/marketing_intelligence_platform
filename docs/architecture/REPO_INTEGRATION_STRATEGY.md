...
**Service wrapper (P10–P12):** FastAPI/Docker service through **P10c**; **P11** API contract hardening merged; **P12** deterministic SDK/API usage examples merged. **Stage A** synthetic fixtures at `examples/fixtures/stage_a/` plus **Stage A.2** loader helpers (`mip.examples.stage_a_fixtures`) provide canonical demo inputs without MMM/GeoX execution outputs.
