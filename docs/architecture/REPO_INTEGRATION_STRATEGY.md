...
**Service wrapper (P10):** [P10_FASTAPI_DOCKER_WRAPPER_PLAN.md](../service/P10_FASTAPI_DOCKER_WRAPPER_PLAN.md) defines a thin FastAPI/Docker boundary over deterministic MIP helpers. **P10a** (`mip.service`) implements `GET /health` and `GET /version`. **P10b** adds deterministic `POST` workflow routes over demo fixture keys. **P10b.1** tightens the service boundary: routes call `mip.workflows.*` directly; `app.demo_fixtures` supplies sample inputs only ([DETERMINISTIC_USAGE_MODES.md](../service/DETERMINISTIC_USAGE_MODES.md)). **P10c** adds a local Docker image for FastAPI smoke testing only ([P10C_DOCKER_SERVICE_SMOKE_REPORT.md](../service/P10C_DOCKER_SERVICE_SMOKE_REPORT.md)) — not Streamlit, not production hosting. Hosted API hardening (P11) deferred. Streamlit remains the canonical public demo.
