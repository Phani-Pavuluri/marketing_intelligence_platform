...
> **Groq compatibility remediation (2026-07-17):** [MIP_LLM_GROQ_PROVIDER_COMPATIBILITY_REMEDIATION_001](../architecture/MIP_LLM_GROQ_PROVIDER_COMPATIBILITY_REMEDIATION_001.md) is `GROQ_PROVIDER_BLOCKED_BY_STRUCTURED_OUTPUT_COMPATIBILITY`. Plain, minimal, and reduced structured probes pass; the full provider wire schema remains rejected. Next artifact: `MIP_LLM_GROQ_PROVIDER_TRANSPORT_REMEDIATION_001`.

> **Groq wire-schema remediation:** [MIP_LLM_GROQ_PROVIDER_WIRE_SCHEMA_REMEDIATION_001](../architecture/MIP_LLM_GROQ_PROVIDER_WIRE_SCHEMA_REMEDIATION_001.md) supersedes the generic transport recommendation. Direct v2 wire parsing passes, but strict wire-to-domain mapping blocks the front-door gate.
...
- CF4 complete: `MIP_LLM_READ_ONLY_CONVERSATIONAL_FRONT_DOOR_001`; provider-neutral read-only front-door seam, fake-provider coverage, structured truth/retrieval context, and deterministic fallback are implemented. Next authorized artifact: `MIP_LLM_CONVERSATIONAL_FRONT_DOOR_EVALUATION_GATE_001`.
- CF5 evaluated: `MIP_LLM_CONVERSATIONAL_FRONT_DOOR_EVALUATION_GATE_001`; automated/fake-provider evidence is scoped, but release is blocked because no concrete SDK-backed provider adapter exists. Phase F remains paused; next remediation: `MIP_LLM_CONCRETE_PROVIDER_ADAPTER_REMEDIATION_001`.
- Provider remediation complete: `MIP_LLM_CONCRETE_PROVIDER_ADAPTER_REMEDIATION_001`; official OpenAI Responses structured-output adapter is wired with bounded, lazy, fail-closed configuration. Live-provider and browser acceptance remain pending; next artifact: `MIP_LLM_FRONT_DOOR_LIVE_PROVIDER_AND_BROWSER_ACCEPTANCE_001`.
- Groq provider adapter complete: `MIP_LLM_GROQ_HOSTED_OPEN_WEIGHT_PROVIDER_ADAPTER_001`; strict Groq models use the official OpenAI SDK without `store` or tools. Live Groq/public-demo acceptance remains pending; next artifact: `MIP_LLM_GROQ_LIVE_PROVIDER_AND_PUBLIC_DEMO_ACCEPTANCE_001`.
